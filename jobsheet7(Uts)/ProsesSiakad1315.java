...
public class ProsesSiakad1315 {
    public static void main(String[] args) {
        ...
        Mahasiswa1315[] mhs = {
            ...
            new Mahasiswa1315("22003", "Citra Dewi", "Informatika")//instansiasi
        };
            ...
            if (pilihan == 1) {
                ...
                for (int i = 0; i < penilaians.length - 1; i++) {
                    ...
                    for (int j = i + 1; j < penilaians.length; j++) {
                        if (penilaians[j].nilaiAkhir > penilaians[maxIdx].nilaiAkhir) {//perbandingan
                            maxIdx = j;//jadi ketita nilai akhir dari array penilaian j lebih besar daripada nilai akhir penilaian maxidx maka maxidx nya j
                        }
                    }
                    Penilaian1315 temp = penilaians[i];//tempat untuk meletakkan nilai sementara
                    penilaians[i] = penilaians[maxIdx];
                    penilaians[maxIdx] = temp;//sorting
                }
                System.out.println("Data Penilaian Setelah Diurutkan:");
                for (Penilaian1315 p : penilaians) {
                    p.tampilPenilaian();//karena kami lihat kelas diagram yang ada di jb tidak memiliki method selecsion sort dan linear search (kami ngikkutin kelas jobsheet aja)
                }
            } else if (pilihan == 5) {
                //Linear Search
